...
class SideMenuVC: UIViewController {
  ...
  let menuSections: [[menuItem]] = [
    [menuItem("List", UIImage(named:"catalog")!, nil),
      menuItem("Bookings", UIImage(named:"bookmarks")!, nil),
      menuItem("Bonuses", UIImage(named:"channels")!, nil)
    ]
  ]
...
//    }
  ...
  func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
    ...
    case 1:
      let bookingsVC = storyboard?.instantiateViewControllerWithIdentifier("Bookings") as! BookingsViewController
      slideMenuController()?.changeMainViewController(bookingsVC, close: true)
    case 2:
      let bonusesVC = storyboard?.instantiateViewControllerWithIdentifier("Bonuses") as! PromoViewController
      slideMenuController()?.changeMainViewController(bonusesVC, close: true)
    default:
      break
    }
